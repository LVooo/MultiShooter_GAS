...
void UCombatComponent::AttachActorToLeftHand(AActor* ActorToAttach)
{
	...
	bool bUserPistolSocket =
		EquippedWeapon->GetWeaponType() == EWeaponType::EWT_Pistol ||
			EquippedWeapon->GetWeaponType() == EWeaponType::EWT_SubmachineGun;
	FName SocketName = bUserPistolSocket ? FName("PistolSocket") : FName("LeftHandSocket");
	// 获取mesh上的socket
...
{
	if (Character && WeaponToEquip == nullptr) return;
...
void UCombatComponent::OnRep_EquippedWeapon()
{
	...
	{
		// 为了确保客户端上的执行顺寻是先设置武器状态再与角色进行碰撞检测
		EquippedWeapon->SetWeaponState(EWeaponState::EWS_Equipped);
		AttachActorToRightHand(EquippedWeapon);
		// 装备武器后禁用人物方向跟随移动方向
		Character->GetCharacterMovement()->bOrientRotationToMovement = false;
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	/*
	// 使用三元运算符进行安全转换
	Controller = Controller == nullptr ? Cast<AAuroraPlayerController>(Character->Controller) : Controller;
	if (Controller)
	{
		// 可以通过Controller获得HUD
		HUD = HUD == nullptr ? Cast<AAuroraHUD>(Controller->GetHUD()) : HUD;
		if (HUD)
		{
			if (EquippedWeapon)
			{
...
void UCombatComponent::InitializerCarriedAmmo()
{
